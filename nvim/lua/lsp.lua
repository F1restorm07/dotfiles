...
-- everything below until L is taken from https://gist.github.com/OXY2DEV/645c90df32095a8a397735d0be646452
...
lsp_hover.hover = function (error, result, context, _)
	...
	if error then
		...
		-- }, true, {})
                vim.notify(error.message, vim.log.ERROR)
	end
	...
	elseif not result or not result.contents then
		...
		-- }, true, {})
                vim.notify("No Information available!", vim.log.INFO)
		return;
	end
...
-- Signature help and renamer taken from NvChad/ui

L.sig_setup = function(client, bufnr)
